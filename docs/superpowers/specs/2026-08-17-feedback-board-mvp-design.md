...
- Local development runs PostgreSQL 18 in a Docker container described by a committed `compose.yaml`.
...
Production reads two connection strings. The application uses Neon's pooled endpoint through one module-scoped node-postgres pool. Vercel Fluid Compute runs concurrent invocations within one function instance and lets them share that pool, so setting `max: 1` would serialize database work that the instance could otherwise perform concurrently. It would not cap the deployment's total connections either: sustained contention can make Vercel add instances, each with its own pool. The MVP therefore leaves node-postgres at its bounded default maximum of ten connections per instance. Ten is a starting point rather than an invariant; if the deployed workload outgrows it, the pool is tuned against observed wait time and the database connection budget instead of being forced to one. Migrations use the direct endpoint instead: they issue DDL and take locks that a transaction-mode pooler does not carry reliably across statements. The local container needs neither endpoint distinction, which is why it belongs to deployment rather than to separate application code paths.

Hosting the database next to the application is a question that only opens if the application itself moves to a VPS, and it may well stay answered no. It is specifically not worth doing while the application runs on Vercel: Vercel deployments egress from arbitrary IP addresses unless the project buys Static IPs or Secure Compute, neither of which exists on the Hobby plan, so a self-hosted database would have to accept connections from the entire internet and defend itself with credentials alone. Fluid Compute reduces the number of instances by reusing each one for concurrent invocations, but Vercel can still scale out and every active instance owns a pool. The connection budget is therefore still deployment-wide, and Neon's managed pooler keeps PostgreSQL's fixed `max_connections` from becoming a direct limit on application concurrency without sacrificing concurrency inside each instance.
...
Environment configuration uses `DATABASE_URL`, `DATABASE_URL_UNPOOLED`, `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL`, and the task-specific `FEEDBACK_BOARD_ENV` marker. `DATABASE_URL_UNPOOLED` is the migration connection described above; locally both point at the same container. Test database reset additionally requires `ALLOW_FEEDBACK_BOARD_TEST_RESET=true`. A committed `.env.example` documents these names without values.
