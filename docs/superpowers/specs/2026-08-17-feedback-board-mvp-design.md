...
Production reads two connection strings. The application uses Neon's pooled endpoint, with the client-side pool held to a single connection, because each serverless instance is its own process and a larger pool multiplies connections against a fixed `max_connections` even behind a pooler. Migrations use the direct endpoint instead: they issue DDL and take locks that a transaction-mode pooler does not carry reliably across statements. The local container needs neither, which is why the distinction belongs to deployment rather than to application code.
